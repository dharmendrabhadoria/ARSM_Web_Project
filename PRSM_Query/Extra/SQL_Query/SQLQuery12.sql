...
Begin                  
 ...
 And                   
  ...
  Declare @InvoiceYear As Int;                  
                Select @InvoiceNo = Isnull((Max(n_InvoiceNo) + 0),                 
   ...
   Select @InvoiceYear =  CONVERT(Int,Substring(CONVERT(varchar,(Select MAX(n_InvoiceNo)  From tbl_InvoiceSummary)),1,4))                
    ...
    Begin         
 declare @mayentrycount int = 0      
 select @mayentrycount = count(0) from tbl_invoicesummary where       
 n_monthyear =  convert( int , datepart(year,getdate())  * 100 + 4)         
 if(@mayentrycount = 0)             
  Select @InvoiceNo = Convert (Int, Convert (Varchar, DATEPART(YEAR, GETDATE())) + '000000');                  
    End                
   ...
   Insert Into #TempTaxes(TaxName ,TaxValue)                   
    ...
    Declare @FromDate As Date;                  
                ...
                Select @FromDate = Convert (Date, Substring(Convert (Varchar, @n_MonthYear), 1, 4) + '-' + Substring(Convert (Varchar, @n_MonthYear), 5, 6) + '-' + '16'),                  
    ...
    Insert Into #tbl_InvoiceSummary(d_ModifiedDate ,n_MonthYear      ,n_WareHouseId,                  
       ...
       Select     GetDate()    ,@n_MonthYear   ,@pn_WareHouseId  ,                  
         TC.n_CustomerId  ,Isnull(TPL.n_PickupAddressId,TC.n_PickupAddressId)  ,@n_UserId                  From  #TempxmlCustomerIDs    As TC                   
         Left Join                   
         ( Select                  
  ...
  And                   
       TB.d_ModifiedDate <= @ToDate                  
         And                   
       TB.n_BoxId Not In ( Select     --Commentd by Sandeep 22 Apr 2015              
--TB.n_BoxId  In ( Select                   
          n_BoxId                  
         From   tbl_BoxDetails                  
   ...
   Update #tbl_InvoiceSummary                   
    ...
    Update #tbl_InvoiceSummary                   
     ...
     n_InvoiceNo    =   @InvoiceNo +Id --CONVERT(Int, @RunningInvoiceNo)+Id                
