...
If Exists (Select 1
           From   sysobjects
           Where  Type = 'P'
                  And name = 'usp_InsertClientData')
    Begin
        Drop Procedure usp_InsertClientData;
    End
...
Create Procedure usp_InsertClientData  
As  
Begin  
 ...
 Set  @CustCount = 1            
  ...
  Insert Into #TempBOXD ( BoxId,     BoxBarCode ,BoxLocId ,BoxLocCode)  
    ...
    Set @n_WorkOrderNo =  (Select Top 1 n_WorkorderNo  
           From tbl_WorkOrder  
            Where n_CustomerId = @n_CustomerId Order by n_WorkorderNo desc )  
    Select  top 1 @n_WoActivityId = n_WoActivityId  From tbl_WoActivities   
 ...
 Set @CustCount = @CustCount+ 1            
    End  
     Update  tbl_FileDetails Set s_FileName = T.[FILENAME 1] ,  
